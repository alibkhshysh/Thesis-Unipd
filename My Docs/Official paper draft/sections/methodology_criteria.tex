...
primary population: M3 cadence and M4 ownership/concentration are evaluated from \path{out/metrics_window_dev_human.csv}
and \path{out/ownership_summary_human.csv}. Bot summaries are reported as sensitivity evidence rather than silently mixed
into maintainer-behavior claims.
...
\path{out/metrics_window_dev_normalized.csv}, \path{out/metrics_window_totals_human_normalized.csv}, \path{out/window_duration_flags.csv} \\
C2 Interpretability &
...
\path{out/metrics_window_dev_human.csv}, \path{out/ownership_dev_shares_human.csv}, \path{out/bot_summary_by_project.csv} \\
C4 Long-term suitability &
Informativeness over time, lifecycle robustness, and incremental update feasibility. &
\path{out/loc_per_tag.csv}, \path{data/windows.csv}, feasibility rubric dimensions \\
\bottomrule
\end{tabularx}
