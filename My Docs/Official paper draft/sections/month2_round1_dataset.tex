...
frozen tags sorted chronologically from oldest to newest. Seven windows are then defined as
...
gives each metric value a replayable repository state and a deterministic release interval.
...
\midrule
brew & \verb|^\d+\.\d+\.\d+$| & Plain semantic-version release tags. \\
click & \verb|^\d+\.\d+\.\d+$| & Plain semantic-version release tags. \\
curl & \verb|^curl-\d+_\d+_\d+$| & Stable curl releases use a \texttt{curl-} prefix and underscores; release candidates are excluded. \\
flask & \verb|^\d+\.\d+\.\d+$| & Plain semantic-version release tags. \\
gin & \verb|^v\d+\.\d+\.\d+$| & Stable Go module tags use a \texttt{v} prefix. \\
junit5 & \verb|^r\d+\.\d+\.\d+$| & Stable JUnit tags use an \texttt{r} prefix; milestone and release-candidate tags are excluded. \\
prettier & \verb|^\d+\.\d+\.\d+$| & Stable releases use plain semantic versions; alpha and non-version tags are excluded. \\
requests & \verb|^v\d+\.\d+\.\d+$| & Stable release tags use a \texttt{v} prefix. \\
tokio & \verb|^tokio-\d+\.\d+\.\d+$| & The repository contains crate-specific tags; only main Tokio runtime releases are included. \\
urllib3 & \verb|^\d+\.\d+\.\d+$| & Plain semantic-version release tags. \\
\bottomrule
\end{tabularx}
...
not corrected away: the freeze follows the declared rule of latest stable repository tags, and
the short-window sensitivity outputs document the effect of the resulting release cadence.
