...
After completing C1--C4, the feasibility step tests whether the comparatively useful metrics can also work inside
...
The feasibility pass has two layers. First, it combines data-derived C1 evidence with structured C2--C4 assessments
into one metric-comparison table.
...
\path{out/feasibility_summary_all.csv} & 5 & Metric-level feasibility totals, exclusions, and implementation accounts. \\
...
M5 & Maintenance responsiveness & 0.0 & 6 & 3 & 4 & 3.25 & Design extension excluded from the current comparison decision. \\
\bottomrule
\end{tabularx}
...
Table~\ref{tab:phase2_combined_comparison} shows the comparison trade-off. M4 has the strongest implemented reputation-oriented profile because
...
the contract to traverse a repository or recompute a metric on-chain. Their feasible implementation approach is to compute
...
The final decision-input table combines the C1--C4 comparison average with the feasibility score. The generated CSV retains
the historical column name \texttt{empirical\_average\_score}, although only C1 is directly data-derived. For ranking eligible metrics,
the analysis uses a transparent weighting: 60\% comparison average and 40\% smart-contract feasibility. This
...
\begin{figure}[H]
\centering
\includegraphics[width=0.92\textwidth]{figures/month3_feasibility/poc_candidate_decision_inputs_corrected.pdf}
\caption{Criterion comparison average, feasibility, and combined readiness scores used for the final selection gate.
Only C1 is data-derived; C2--C4 are structured author assessments.}
\label{fig:poc_candidate_readiness}
\end{figure}

The evidence identifies M4 ownership/concentration as the strongest eligible candidate for the final selection
gate. This does not mean that M3 and M2 are discarded. Instead, they remain supporting evidence and safeguards:
M3 helps interpret activity continuity, and M2 helps check whether ownership changes are backed by actual change effort.
M1 remains necessary for context and normalization. M5 is reserved for future work.

Section~\ref{sec:phase2_poc_metric_selection} records the final metric-selection rationale, and Phase III instantiates the
selected metric in the DESTINO-aligned PoC.
