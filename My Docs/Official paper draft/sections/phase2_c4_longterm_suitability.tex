...
The fourth Phase II criterion evaluates whether each metric remains useful when the project history becomes longer.
...
while the reputation-fit dimension records whether the metric remains meaningful as a long-term signal. The
dimensions \texttt{C4\_D1}--\texttt{C4\_D5} are local to the C4 long-term suitability criterion; they are not
the same as the later feasibility dimensions D1--D10.

Historical coverage counts are derived from the dataset, while the dimension scores and long-term interpretations are
structured author assessments. With seven original windows per project and heterogeneous history spans, C4 supports a
comparative design judgment but does not establish multi-year predictive validity.
...
all 80 tag snapshots. M2 and M3 use 1,063 human-only project-window-developer rows. M4 uses the same human-only
...
M5 & Maintenance responsiveness & Design extension & 4/10 & Low & Conceptually useful, but not eligible for the current comparison decision without event data. \\
\bottomrule
\end{tabularx}
...
The next section applies the smart-contract feasibility scoring pass. That pass combines the C1--C4 findings with
the predefined feasibility rubric before the final proof-of-concept metric is selected.
