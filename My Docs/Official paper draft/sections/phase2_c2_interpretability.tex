...
The second Phase II criterion evaluates whether each candidate metric can be explained clearly and audited
...
The dimensions \texttt{C2\_D1}--\texttt{C2\_D5} are local to the C2 interpretability criterion; they are not
the same as the later feasibility dimensions D1--D10. The rubric is generated as a CSV artifact so that the
score basis remains explicit.

The numerical C2 ratings are structured author assessments encoded as constants with written rationales. The script
reproduces the rubric and totals; it does not infer interpretability from repository observations or provide independent
rater agreement. C2 is therefore a transparent decision aid rather than an empirical measurement.
...
M1 & Snapshot size & Implemented & 8/10 & High & Straightforward to explain and reproduce, but mainly useful as context rather than reputation evidence. \\
...
M5 & Maintenance responsiveness & Design extension & 6/10 & Medium & Conceptually useful, but excluded from the current comparison decision because issue/PR event data were not collected. \\
\bottomrule
\end{tabularx}
...
with the implemented candidates in the current comparison decision.
...
The next section evaluates C3 gaming resistance. That step focuses on how easily each metric can be manipulated
